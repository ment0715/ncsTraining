package stg.action.akts;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import stg.common.Action;
import stg.common.ActionForward;
import stg.service.AktsService;
import stg.vo.AktsVo;


public class AktsTrainingAction implements Action{
	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 애플리케이션테스트수행 공부하러가기 
		
		// 문제 가져오기
		AktsService svc = new AktsService();
		AktsVo vo = svc.getQuestion();
		
		request.setAttribute("aktsVo", vo);
		
		
		ActionForward forward = new ActionForward();
		forward.setPath("views/module/akts/aktsTraining.jsp");
		return forward;
	}
}
